import {
  AskPassphraseCallback,
  LocalAccountSigner,
  NONCE,
} from "./LocalAccountSigner";
...
export const getPolkadotAccounts = async () => {
  try {
    ...
    if (accounts.length > 1) {
      alert("Multiple wallets found. Using the first one.");
    }

    const { name, address, signer, signRaw } = accounts[0];
...
export const getMetamaskAccount = async () => {
  try {
    ...
    if (!accounts || accounts.length === 0) {
      alert(
        "No accounts found. Please ensure MetaMask is unlocked and accounts are available."
      );
